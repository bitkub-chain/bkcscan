...
node {
	...
	stage('Create Temp Config') {
	    ...
	    sshCommand remote: remote, command: 'docker container stop bkc-explorer; docker container rm bkc-explorer; docker ps -a | grep Exited | awk \'{print $1}\' | xargs docker container rm; sleep 1;'
	}
